...
// row via onSelect. No new UI primitive/library — just FormInputs' input
// styling plus a small floating list, scoped per row instance.
//
// The dropdown is positioned with `position: fixed` (computed from the
// input's own bounding rect) rather than `absolute` — this row lives
// inside the product table's `overflow-x-auto` wrapper, and setting
// overflow-x on a container forces the browser to also compute overflow-y
// as `auto` (a CSS-spec rule: an element can't have one axis "visible" and
// the other not), which silently clips an absolutely-positioned dropdown
// even though it's still correctly rendered in the DOM. `fixed` escapes
// that clipping, matching the same pattern the 3-dot row-action menus use
// elsewhere in Inventory (e.g. ProductList.jsx).
function ProductPicker({ products, value, onSelect, placeholder }) {
