...
import {
  ...
  CheckCircle2, Eye, Edit2, XCircle, MoreVertical, UserCog, Wrench, Trash2,
} from 'lucide-react'
...
const HW_ITEM_SUGGESTIONS = ['Intercom Panel Unit', 'Handset Unit', 'Power Adapter', 'Other']
const WIRE_ITEM_SUGGESTIONS = ['Drop Wire (m)', 'Cable', 'Other']
function newHwRow(name = '', qty = '', unit = 'pcs')   { return { id: Date.now() + Math.random(), name, qty, unit } }
function newWireRow(name = '', qty = '', unit = 'pcs') { return { id: Date.now() + Math.random(), name, qty, unit } }
...
function AssignEngineerModal({ isOpen, onClose, order, onSubmit }) {
  ...
  useEffect(() => {
    if (isOpen && order) {
      ...
      setErrors({})
      setHwToggle(true)
      setHwItems(order.hwItems?.length ? order.hwItems : [newHwRow('Intercom Panel Unit', '1', 'pcs')])
      setWireItems(order.wireItems?.length ? order.wireItems : [newWireRow('Drop Wire (m)', '50', 'pcs')])
    }
  }, [isOpen, order])
...
export default function IntercomInstallations() {
  ...
  const EMPTY_DRAFT = { status: '', engineer: '', zone: '', dateFrom: '', dateTo: '' }
  ...
  function openDrawer() {
    setDraft({
      status: filterStatus, engineer: filterEngineer, zone: filterZone,
  ...
  function clearAllFilters() {
    setFilterStatus(''); setFilterEngineer(''); setFilterZone('')
    setFilterDateFrom(''); setFilterDateTo('')
    setDraft(EMPTY_DRAFT)
    setPage(1)
  }

  const filterCount = [filterStatus, filterEngineer, filterZone, filterDateFrom || filterDateTo].filter(Boolean).length
  ...
  function handleAssignSubmit({ engineers, hwItems, wireItems }) {
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage intercom installation work orders</p>
        </div>
        <Button size="sm" icon={<Plus size={14} />}>New Installation</Button>
      </div>

      {/* ── Stats ──────────────────────────────────────────────────────────── */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total',       value: stats.total,      icon: ClipboardList, color: 'text-brand-blue',   bg: 'bg-brand-blue/10'   },
          { label: 'Pending',     value: stats.pending,    icon: Clock,         color: 'text-brand-orange', bg: 'bg-brand-orange/10' },
          { label: 'In Progress', value: stats.inprogress, icon: Loader2,       color: 'text-blue-600',     bg: 'bg-blue-100'        },
          { label: 'Completed',   value: stats.completed,  icon: CheckCircle2,  color: 'text-emerald-700',  bg: 'bg-emerald-100'     },
        ].map(s => {
          const Icon = s.icon
          return (
        ...
        {(hasActiveFilters || search.trim()) && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {filterEngineer && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">
                {filterEngineer}
                <button onClick={() => setFilterEngineer('')} className="ml-0.5 hover:text-purple-900"><X size={10} /></button>
              </span>
            )}
            {filterZone && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">
                  ...
                  return (
                    <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3 whitespace-nowrap">
                        <button
                          onClick={() => navigate(`/intercom/installations/${o.id}`)}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="font-medium text-gray-800 whitespace-nowrap">{o.customer}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <button
                          onClick={e => openMenu(e, o.id)}
                          className={`w-7 h-7 flex items-center justify-center rounded-lg transition-colors ${
                            menuId === o.id ? 'bg-gray-100 text-gray-700' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'
                          }`}
                        >
      ...
      <div className={`fixed top-14 left-0 right-0 bottom-0 z-50 transition-opacity duration-300 ${drawerOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute right-0 top-0 h-full w-80 bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
            ...
            {/* Engineer */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Engineer</label>
              <div className="relative">
            ...
            {/* Date Range */}
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Installation Date</label>
              <div className="space-y-2">
                <div>
