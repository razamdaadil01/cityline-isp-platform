...
import {
  getRecoveries, subscribeRecoveries, updateRecovery, RECOVERY_STATUS_CFG, RECOVERY_TERMINAL_STATUSES,
} from '../data/customerRecoveryStore'
import { resolveRecovery } from '../data/userAssignmentStore'
import { updateCustomer } from '../data/customersData'

// Phase 4 of the Customer Disconnection flow (customerRecoveryStore.js) —
// mirrors IntercomHardwareRecovery.jsx's list/stats/status-change pattern
// closely, but for core RES-/ENT- customers via a wholly separate store.
// The one real behavioral difference: resolving a work order here also
// closes out the linked userAssignmentStore.js handoff(s) (Step 3's
// "disconnection" stub) and stamps the outcome onto the customer record
// itself, so CustomerDetail.jsx and the Customers list can both see it
// without re-deriving it from this store.

const RECOVERY_OUTCOME = {
  completed: 'returned',
  missing_hardware: 'missing',
  damaged_hardware: 'damaged',
...
export default function CustomerHardwareRecovery() {
  ...
  function handleStatusChange(id, status) {
    ...
    if (RECOVERY_TERMINAL_STATUSES.includes(status)) {
      // Step 3: close out the linked handoff(s) and record what actually
      // happened, so Phase 4 (billing/settlement) has something to charge
      // for on missing/damaged hardware later.
      resolveRecovery(id, RECOVERY_OUTCOME[status])
      const r = getRecoveries().find(o => o.id === id)
      if (r) updateCustomer(r.customerId, { hardwareRecoveryOutcome: RECOVERY_OUTCOME[status] })
    }
  }
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Technician visits scheduled to recover hardware from disconnecting customers</p>
      </div>
