...
export default function OutageDetail() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-6xl mx-auto">
      ...
      {/* Header — same outer/actions classes as Ticket Detail's header row */}
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-gray-900 font-mono truncate">{outage.id}</h1>
            <p className="text-sm text-gray-500 mt-0.5 truncate">{outage.title}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <Button variant="secondary" icon={<Bell size={13} />} onClick={() => setNotifyOpen(true)}>
            Notify Customers
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
              ...
              {activeTab === 'incident' && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-bold text-gray-800 uppercase tracking-wider">Outage Details</p>
                ...
                <div>
                  ...
                  {linkedTickets.length === 0 ? (
                    ...
                    <div className="border border-surface-border rounded-lg divide-y divide-surface-border overflow-hidden">
                      {linkedTickets.map(t => (
                        <div key={t.id} className="flex items-center justify-between gap-4 px-4 py-3 hover:bg-gray-50 cursor-pointer transition-colors"
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-surface-border">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-xs font-bold text-gray-800 uppercase tracking-wider">Notification Log</p>
        ...
        {/* ── RIGHT SIDEBAR: Impact Summary — same card treatment as Ticket Detail's sidebar ── */}
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-surface-border p-5 shadow-card">
            ...
            <div>
              <p className="text-[11px] text-gray-400 uppercase tracking-wide mb-2">Affected NAS Port ID(s)</p>
              {(outage.affectedNasPorts ?? []).length === 0 ? (
                <p className="text-xs text-gray-300 text-center py-3">—</p>
              ) : (
                <div className="space-y-2">
                  {outage.affectedNasPorts.map((p, i) => (
                    <div key={i} className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-gray-50 border border-surface-border">
                      <MapPin size={12} className="text-brand-blue/60 shrink-0" />
                      <span className="font-mono text-xs text-brand-blue font-semibold truncate">{p}</span>
                    </div>
                  ))}
                </div>
              )}
