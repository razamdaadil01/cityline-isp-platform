...
import { MOCK_BANDWIDTHS, MOCK_TENURES, MOCK_OTT, MOCK_BW_PACKAGES, MOCK_OTHER_PACKAGES } from '../data/packagesStore'
...
export default function PackageEdit() {
  ...
  const [pkgType, setPkgType] = useState(pkg.type || '')
  const [pkgName, setPkgName] = useState(pkg.name || '')

  // Step 2 — bandwidth
  const [bwRows, setBwRows] = useState(isBw && pkg.rows?.length ? pkg.rows.map(r => ({ ...r })) : [{ ...EMPTY_ROW }])

  // Step 2 — other
  const [bound, setBound] = useState(!!(pkg.bindPackage))
  const [bindPkg, setBindPkg] = useState(pkg.bindPackage || '')
  const [othPrice, setOthPrice] = useState(pkg.price ? String(pkg.price) : '')
  const [separateInvoice, setSeparateInvoice] = useState(pkg.separateInvoice || false)

  // Step 3
  const [editable, setEditable] = useState(pkg.editable || false)
  const [landline, setLandline] = useState(pkg.landline || false)
  const [offer, setOffer] = useState(pkg.offer || false)
  ...
  const handleSave = () => {
    setToast(true)
    setTimeout(() => { navigate(`/packages/${packageId}`) }, 1500)
  }
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      {toast && (
        <div className="fixed top-6 right-6 z-50 bg-green-600 text-white px-5 py-3 rounded-xl shadow-xl flex items-center gap-2 text-sm font-medium">
          <CheckCircle size={16} /> Package updated successfully!
        </div>
      )}

      <div>
        <h1 className="text-2xl font-bold text-[#0F2744]">Edit Package</h1>
        <p className="text-sm text-gray-500 mt-0.5">{pkg.id} · Step {step + 1} of 3 — {STEPS[step]}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
        ...
        {step === 1 && pkgType === 'Bandwidth' && (
          <>
            <p className="text-sm text-gray-500">Edit pricing rows — each row is a bandwidth/tenure combination.</p>
            ...
            <button onClick={addRow} className="flex items-center gap-1.5 text-[#0A8DCD] text-sm font-medium hover:text-blue-600">
            ...
            {bound ? (
              ...
              <div className="space-y-3">
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 text-sm text-blue-700">
                  Standalone package — can be added independently to any lead
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Price (&#8377;) *</label>
                  <input type="number" value={othPrice} onChange={e => setOthPrice(e.target.value)} className={inp} placeholder="0" />
        ...
        {step === 2 && (
          <>
            {[
              { key: 'editable', val: editable, set: setEditable, title: 'Is Package Editable?',        icon: '✏️', desc: 'Allow sales team to modify price during lead stage' },
              { key: 'landline', val: landline, set: setLandline, title: 'Landline Number Applicable?', icon: '📞', desc: 'Does this package support landline/VOIP assignment?' },
              { key: 'offer',    val: offer,    set: setOffer,    title: 'Is Offer Package?',           icon: '🎁', desc: 'Mark as introductory/special offer' },
            ].map(opt => (
      ...
      <div className="flex justify-between">
        <button onClick={() => step === 0 ? navigate(`/packages/${packageId}`) : setStep(s => s - 1)}
        ...
        {step < 2 ? (
          ...
          <button onClick={handleSave}
            ...
            <CheckCircle size={15} /> Update Package
          </button>
        )}
      </div>
    </div>
