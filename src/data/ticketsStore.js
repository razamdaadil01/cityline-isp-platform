...
export const OFFICE_TEAMS = ['Team A', 'Team B', 'Team C']
...
//
// userId bridges each profile to userStore.js's canonical Technician
// roster (role='engineer' users — see that file's INITIAL_USERS comment),
// same as installationsStore.js's FIELD_ENGINEERS does. These 5 names never
// collided with an existing user (unlike FIELD_ENGINEERS' eng-002/003/005)
// — u11-u15 were created fresh with these exact names specifically to back
// this list, closing the gap the technician-dashboard audit found ("two
// disjoint rosters that never intersect by name").
export const TECHNICIAN_PROFILES = [
  { name: 'Suresh Iyer', skills: ['Fiber Splicing', 'OLT/Network Diagnostics'], active: true, distanceFromCustomer: 1.5, userId: 'u11' },
  { name: 'Prakash Yadav', skills: ['Cabling & Wiring', 'General Troubleshooting'], active: true, distanceFromCustomer: 0.8, userId: 'u12' },
  { name: 'Manoj Verma', skills: ['ONU Replacement', 'Fiber Splicing'], active: true, distanceFromCustomer: 5.1, userId: 'u13' },
  { name: 'Dinesh Kumar', skills: ['Router Configuration', 'General Troubleshooting'], active: true, distanceFromCustomer: 2.4, userId: 'u14' },
  { name: 'Vikram Singh', skills: ['Fiber Splicing', 'General Troubleshooting'], active: true, distanceFromCustomer: 3.7, userId: 'u15' },
]
...
const SEED = [
  {
    ...
    area: 'Andheri West',
    createdAt: new Date(NOW - 6 * H).toISOString(), updatedAt: new Date(NOW - 0.5 * H).toISOString(),
    ...
    activityLog: [
      ...
      { time: new Date(NOW - 5.3 * H).toISOString(), actor: 'System', action: 'Assigned to Team A' },
      { time: new Date(NOW - 4 * H).toISOString(), actor: 'Ravi T.', action: 'Comment added — "Advised customer to power-cycle ONU, issue persists, escalating."' },
      { time: new Date(NOW - 3 * H).toISOString(), actor: 'Ravi T.', action: 'Technician visit scheduled with Suresh Iyer for ' + new Date(NOW + 18 * H).toISOString().slice(0, 10) + ' 11:00' },
      { time: new Date(NOW - 1 * H).toISOString(), actor: 'Ravi T.', action: 'Status changed to In Progress' },
      { time: new Date(NOW - 0.5 * H).toISOString(), actor: 'System', action: 'Customer notified via WhatsApp' },
    ],
    communicationLog: [
  ...
  },
  {
    ...
    officeTeam: 'Team A', branch: 'Bandra Branch', customerType: 'Corporate',
    category: 'Billing', subcategory: 'Invoice Query',
    priority: 'P2', status: 'Assigned',
    assignedAgent: 'Neha M.', assignedTechnician: null,
    area: 'Bandra West',
    createdAt: new Date(NOW - 5 * H).toISOString(), updatedAt: new Date(NOW - 0.5 * H).toISOString(),
    ...
    activityLog: [
      { time: new Date(NOW - 5 * H).toISOString(), actor: 'System', action: 'Ticket created' },
      { time: new Date(NOW - 4.8 * H).toISOString(), actor: 'System', action: 'Status changed to Assigned' },
      { time: new Date(NOW - 4.75 * H).toISOString(), actor: 'System', action: 'Assigned to Neha M. (Team A)' },
      { time: new Date(NOW - 4 * H).toISOString(), actor: 'Neha M.', action: 'Comment added — "Called to clarify the promotional offer details; customer to share screenshot."' },
      { time: new Date(NOW - 2.5 * H).toISOString(), actor: 'Neha M.', action: 'Comment added — "Customer confirmed screenshot shared, verifying with billing team."' },
      { time: new Date(NOW - 1 * H).toISOString(), actor: 'Neha M.', action: 'Comment added — "Informed customer that the promotional discount will be adjusted in the next invoice cycle."' },
      { time: new Date(NOW - 0.5 * H).toISOString(), actor: 'System', action: 'Customer notified via WhatsApp' },
    ],
    communicationLog: [
  ...
  },
  {
    ...
    activityLog: [
      { time: new Date(NOW - 9 * H).toISOString(), actor: 'System', action: 'Ticket created' },
      { time: new Date(NOW - 8.9 * H).toISOString(), actor: 'System', action: 'Status changed to Assigned' },
      { time: new Date(NOW - 8.85 * H).toISOString(), actor: 'System', action: 'Assigned to Ravi T. (Team A)' },
      { time: new Date(NOW - 8 * H).toISOString(), actor: 'Ravi T.', action: 'Comment added — "Customer escalated again; confirmed technician dispatched to OLT-ANW-02."' },
      { time: new Date(NOW - 7 * H).toISOString(), actor: 'Ravi T.', action: 'Technician visit scheduled with Suresh Iyer for ' + new Date(NOW - 1 * H).toISOString().slice(0, 10) + ' 11:30' },
      { time: new Date(NOW - 6 * H).toISOString(), actor: 'System', action: 'Status changed to In Progress' },
      { time: new Date(NOW - 1 * H).toISOString(), actor: 'Suresh Iyer', action: 'Hardware added: SFP Module (OLT-grade)' },
    ],
    technicianVisit: {
  ...
  },
  {
    ...
    officeTeam: 'Team B', branch: 'Colaba Branch', customerType: 'Corporate',
  ...
  },
  {
    ...
    officeTeam: 'Team A', branch: 'Mahim Branch', customerType: 'Residential',
  ...
  },
  {
    ...
    officeTeam: 'Team A', branch: 'Bandra Branch', customerType: 'Residential',
  ...
  },
  {
    ...
    officeTeam: 'Team C', branch: 'Chembur Branch', customerType: 'Corporate',
  ...
  },
  {
    ...
    officeTeam: 'Team B', branch: 'Colaba Branch', customerType: 'Residential',
